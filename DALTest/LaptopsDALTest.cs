...
namespace DALTest
{
    public class LaptopsDALTest
    {
        ...
        private void GetNameTest(string _name, bool expected)
        {
            ...
            list = lsdal.GetLaptops(LaptopFilter.FILTER_BY_LAPTOP_NAME, laptop1);
        ...
        // [InlineData(3000000, 19000000, true)]
        // [InlineData(3000000, 19000000, true)]
        private void GetPriceTest(int _min, int _max, bool expected)
        {
            Laptop laptop1 = new Laptop() { minPrice = _min, maxPrice = _max };
            List<Laptop> list = new List<Laptop>();
            list = lsdal.GetLaptops(LaptopFilter.FILTER_BY_LAPTOP_PRICE, laptop1);
